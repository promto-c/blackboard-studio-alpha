...
export interface RotoOverlayProps extends ViewportOverlayProps {
  node: RotoNode;
}

function RotoOverlay(props: RotoOverlayProps) {
  const ctx = ecc(props);
  const roto = ctx.roto;
  const viewport = ctx.viewport;
  const interaction = roto.interaction;
  const showOverlays = viewport.showOverlays;
  const cursorOnly =
    !showOverlays && !(viewport.activeViewportTool === 'nudge' || interaction.isAdjustingRadius);
  const node = props.node;
  const frame = props.frame;
  const zoom = props.zoom;
  const selectedRotoPathIds = roto.selectedPathIds;
  const selectedRotoPointRefs = roto.selectedPointRefs;
  const setHierarchySelection = roto.setHierarchySelection;
  const isRotoSelectActive = interaction.isRotoSelectActive as boolean;
  const activeViewportTool = props.activeTool ?? '';
  const altPressed = viewport.altPressed;
  const nudge = roto.nudgeOverlayState as unknown as NudgeOverlayState;
  const rotoTransformSelection =
    interaction.rotoTransformSelection as RotoTransformSelection | null;
  const transformIsDegenerate = interaction.transformIsDegenerate as boolean;
  const transformMoveHandleRadius = interaction.transformMoveHandleRadius as number;
  const transformRotateHitRadius = interaction.transformRotateHitRadius as number;
  const transformHandleSize = interaction.transformHandleSize as number;
  const transformHandleHitSize = interaction.transformHandleHitSize as number;
  const transformHandlePositions =
    interaction.transformHandlePositions as TransformHandlePosition[];
  const transformRotateHandlePoint = interaction.transformRotateHandlePoint as ScenePoint | null;
  const transformInteractionLabel = interaction.transformInteractionLabel as string | null;
  const activeTransformHandle = interaction.activeTransformHandle as TransformHandleKind | null;
  const hoveredTransformHandle = interaction.hoveredTransformHandle as TransformHandleKind | null;
  const affineModifierPressed = viewport.affineModifierPressed;
  const isMoveTransformActive = interaction.isMoveTransformActive as boolean;
  const isMoveTransformHovered = interaction.isMoveTransformHovered as boolean;
  const isRotateTransformActive = interaction.isRotateTransformActive as boolean;
  const isRotateTransformHovered = interaction.isRotateTransformHovered as boolean;
  const beginRotoTransformDrag = interaction.beginRotoTransformDrag as (
    e: React.MouseEvent,
    handle: TransformHandleKind,
  ) => void;
  const setHoveredTransformHandle = interaction.setHoveredTransformHandle as (
    h: TransformHandleKind | null,
  ) => void;
  const hoveredRotoPathId = interaction.hoveredRotoPathId as string | null;
  const setHoveredRotoPathId = interaction.setHoveredRotoPathId as (id: string | null) => void;
  const dragPointState = interaction.dragPointState as unknown | null;
  const hoveredPointInfo = interaction.hoveredPointInfo as {
    pathId: string;
    pointIndex: number;
  } | null;
  const handlePointMouseDown = interaction.handlePointMouseDown as (
    e: React.MouseEvent,
    pathId: string,
    idx: number,
  ) => void;
  const beginPointWeightDrag = interaction.beginPointWeightDrag as (
    e: React.MouseEvent,
    pathId: string,
    pointIndex: number,
    handleNormal: ScenePoint,
  ) => void;
  const setSelectedPointWeightMode = interaction.setSelectedPointWeightMode as (
    pathId: string,
    pointIndices: number[],
    pointWeightMode: RotoPointWeightMode,
  ) => void;
  const setSelectedPointType = interaction.setSelectedPointType as (
    pathId: string,
    pointIndices: number[],
    pointType: RotoPointType,
  ) => void;
  const setHoveredPointInfo = interaction.setHoveredPointInfo as (
    info: { pathId: string; pointIndex: number } | null,
  ) => void;
  const pointWeightDragState = interaction.pointWeightDragState as {
    pathId: string;
    pointIndices: number[];
  } | null;
  const pointWeightControlState = interaction.pointWeightControlState as {
    pathId: string;
    pointIndex: number;
    pointIndices: number[];
  } | null;
  const rotoPointWeightMode = roto.pointWeightMode as RotoPointWeightMode;
  const temporalController = interaction.temporalController as RotoTemporalControllerState | null;
  const onTemporalControllerChange = interaction.setTemporalControllerValue as (
    value: RotoTemporalControllerValue | null,
  ) => void;
  const onTemporalControllerCommit = interaction.commitTemporalController as (
    value: RotoTemporalControllerValue,
  ) => void;
  const motionCueTargetPathIdSet = roto.motionCueTargetPathIdSet;
  const rotoMotionCueEnabled = roto.motionCueEnabled;
  const rotoMotionCueMode = roto.motionCueMode;
  const gradientTrailsByPath = roto.gradientTrailsByPath as unknown as Map<
    string,
    GradientTrailPath[]
  >;
  const speedHeatSegmentsByPath = roto.speedHeatSegmentsByPath as unknown as Map<
    string,
    HeatlineSegment[]
  >;
  const motionBlurCuePathsByPath = roto.motionBlurCuePathsByPath as unknown as Map<
    string,
    MotionBlurCuePath[]
  >;
  const hoveredSegment = interaction.hoveredSegment as {
    pathId: string;
    insertIndex: number;
    point: { x: number; y: number };
  } | null;
  const rotoRefinement = roto.refinement as {
    closed: boolean;
    targetPathId?: string;
    epsilon: number;
  } | null;
  ...
  const isHoveringClosePoint = interaction.isHoveringClosePoint as boolean;
  const marqueeState = interaction.marqueeState as MarqueeState | null;
  const activeTrackingPoints = roto.activeTrackingPoints as { x: number; y: number }[] | null;
  ...
  // ── SVG defs (rendered directly in the SVG, available for url() refs) ──
  const rotoBsplineDrawingState = interaction.bsplineDrawingState;

  return (
    <>
      {rotoBsplineDrawingState?.previewSegment &&
        (() => {
          const sStart = stabilizePoint(
            rotoBsplineDrawingState.previewSegment.start,
            viewport.stabilizationMatrix,
          );
          const sEnd = stabilizePoint(
            rotoBsplineDrawingState.previewSegment.end,
            viewport.stabilizationMatrix,
          );
          return (
            <defs>
                    ...
                    ? (e) => {
                        ...
                        if (e.shiftKey)
                          setHierarchySelection(
                            node.id,
                            [],
                            [...new Set([...selectedRotoPathIds, path.id])],
                          );
                        else setHierarchySelection(node.id, [], [path.id]);
                      }
                    : undefined
                }
              >
