...
export function useRotoInteraction(params: UseRotoInteractionParams) {
  ...
  const handlePointMouseDown = useCallback(
    (e: React.MouseEvent, pathId: string, pointIndex: number) => {
      ...
      if (e.altKey) {
        ...
        if (path) {
          ...
          if (path.points.length > minPoints) {
            ...
            setHierarchySelection(selectedNodeIdVal, [], []);
            setSelectedRotoPointRefs([]);
          }
        }
        return;
      ...
      setHierarchySelection(selectedNodeIdVal, [], nextPathIds);
      setSelectedRotoPointRefs(nextPointRefs);
  ...
  const setSelectedPointWeightMode = useCallback(
    ...
    [nodes, commitMutation, selectedNode, selectedNodeId, updateNode],
  );
  ...
  const cleanupOnToolChange = useCallback(
    (previousTool: string | null) => {
      ...
      if (activeViewportTool !== 'select') {
        if (hoveredTransformHandle) setHoveredTransformHandle(null);
        if (transformDragState) {
  ...
  useEffect(() => {
    if (!rotoTransformSelection && hoveredTransformHandle) {
      setHoveredTransformHandle(null);
    }
  }, [rotoTransformSelection, hoveredTransformHandle]);
  ...
  const deletePointsInNudgeArea = useCallback((): boolean => {
    ...
    commitMutation({
      patch: {},
      history: { label: 'Delete Points in Nudge Area', state: { nodes: newNodes, selectedNodeId } },
    });
    ...
    (
      ...
      if (
        ...
        if (!isDrawing) {
          ...
          startDrawingShape(newPath as RotoPath);
      ...
      // Select tool: marquee
      // Note: e is actually a native MouseEvent (cast via adapter) so
      // e.currentTarget would be the React root, not the viewport div.
      // Use viewportRef.current to check we're clicking the viewport
      // background vs an interactive SVG element (control point).
      if (
        e.button === 0 &&
        activeViewportTool === 'select' &&
        selectedNode?.type === NodeType.ROTO
      ) {
        if (!viewportRef.current || (e.target as Node) !== viewportRef.current) return false;
        e.preventDefault();
    ...
    (e: ViewportMouseEvent): boolean => {
      ...
      if (nudgeDragState) {
        if (nudgeDidMoveRef.current) {
          commitMutation({
            patch: {},
            history: { label: 'Nudge Stroke', state: { nodes, selectedNodeId } },
          });
          ...
          if (didBake) {
            ...
            commitMutation({
              patch: {},
              history: { label: 'Keyframe Nudge Area', state: { nodes: newNodes, selectedNodeId } },
            });
          }
        }
        nudgeDidMoveRef.current = false;
      ...
      if (insertedPointDragState) {
        commitMutation({
          patch: {},
          history: { label: 'Move Roto Point', state: { nodes, selectedNodeId } },
        });
      ...
      if (dragPointState) {
        commitMutation({
          patch: {},
          history: { label: `Move Roto Points`, state: { nodes, selectedNodeId } },
        });
      ...
      if (marqueeState) {
        ...
        if (isClick) {
          if (selectedRotoPointRefs.length > 0) {
            setHierarchySelection(selectedNodeIdVal, [], []);
            setSelectedRotoPointRefs([]);
          } else if (selectedRotoPathIds.length > 0)
            setHierarchySelection(selectedNodeIdVal, [], []);
        } else {
          const mRect = {
          ...
          if (selectedNode?.type === NodeType.ROTO) {
            ...
            if (selectedPathPointHits.length > 0) {
              ...
              setHierarchySelection(
                selectedNodeIdVal,
                [],
                Array.from(new Set(nextPointRefs.map((pointRef) => pointRef.pathId))),
              );
              setSelectedRotoPointRefs(nextPointRefs);
            } else {
      ...
      if (drawingState && selectedNode?.type === NodeType.ROTO) {
        ...
        setHierarchySelection(selectedNodeIdVal, [], [newPath.id]);
        setDrawingState(null);
  ...
  /** Nudge overlay visualization state, consumed by RotoOverlay. */
  const nudgeOverlayState = useMemo(
    () => ({
      activeViewportTool,
      altPressed,
      isAdjustingRadius,
      nudgeDragState,
      radiusAdjustCenter: radiusAdjustStartRef.current?.center ?? null,
      radiusAdjustInitialRadius: radiusAdjustStartRef.current?.initialRadius ?? null,
      mouseScenePos,
      nudgeRadius,
      nudgePreviewPoints,
    }),
