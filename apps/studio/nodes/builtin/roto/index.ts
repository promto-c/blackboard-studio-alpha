...
const rotoAnimation: NodeAnimationBehavior = {
...
export const rotoNode: NodeDefinition = {
  type: NodeType.ROTO,
  name: 'Roto',
  category: 'Effect',
  renderMode: 'mask',
  ...
  ViewportOverlayComponent: RotoOverlay,
  ...
  getOverlayVisibility: (_node, ctx) => {
    const isAdjustingRadius = ctx.roto?.interaction?.isAdjustingRadius === true;
    if (ctx.viewport.showOverlays) return { forceShowSvg: false };
    return {
      forceShowSvg:
        ctx.viewport.activeViewportTool === 'select' ||
        ctx.viewport.activeViewportTool === 'nudge' ||
        isAdjustingRadius,
    };
  },
  toolHotkeys: {
