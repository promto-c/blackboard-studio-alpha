...
export interface NodeAnimationBehavior {
...
export const mediaTransformAnimation: NodeAnimationBehavior = {
  getAnimatableProperties: (node) => {
    const { props, addProp } = createAnimatablePropertyCollector();

    if (!('transform' in node) || !('x' in node.transform) || !('y' in node.transform)) {
      return props;
    }
...
export const uniformSliderAnimation: NodeAnimationBehavior = {
  getAnimatableProperties: (node) => {
    const { props, addProp } = createAnimatablePropertyCollector();
