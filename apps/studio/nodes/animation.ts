...
export type { AnimatablePropertiesOptions, AnimatablePropertyDef } from './animationHelpers';
...
export const setKeyframeValue = (
  ...
  const animation = nodeRegistry.get(node.type)?.animation;
  const newNode =
...
export const getAnimatableProperties = (
  ...
  const effectProps = nodeRegistry
    .get(node.type)
