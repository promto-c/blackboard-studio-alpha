...
import { AnyNode, LensDistortionNode, UniformUIType, AnyUniform } from '@blackboard/types';
...
const LensDistortionAdjustments: React.FC<{ node: AnyNode }> = ({ node: anyNode }) => {
  ...
  const handleReset = (name: string) => () => {
    const defaultUniforms = parseUniformsFromGLSL(LENS_DISTORTION_SHADER);
    ...
    if (uniform.ui === UniformUIType.SLIDER) {
      ...
      return (
        <Slider
          ...
          displayFormatter={(v) => v.toFixed(uniform.step < 0.1 ? 3 : uniform.step < 1 ? 2 : 0)}
